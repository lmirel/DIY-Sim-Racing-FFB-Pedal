...
// and in case, (a) send tuned servo parameters and (b) prepare the servo for signal read
  ...
  Serial.println(stepper->getLimitMin());
  Serial.print("Max Position is ");
  Serial.println(stepper->getLimitMax());
...
// provide joystick output on PIN
    ...
    Serial.print("PU task stack size=");
...
//#define PRINT_SERVO_POS_EVERY_N_CYCLES
        ...
        if ((timeDiff > TIME_SINCE_SERVO_POS_CHANGE_TO_DETECT_STANDSTILL_IN_MS) && (timeNow_l > 0))
        {
          cond_1 = true;
        }
        else
